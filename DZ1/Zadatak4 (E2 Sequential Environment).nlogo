; Izmijeniti E2 - Sequential Environment simulaciju na način da se proizvod automatski pojavljuje na traci nakon
; odrađenog broja otkucaja, a pojavljivanje novih proizvoda zaustavlja kada je serija kompletirana (30 ispravnih proizvoda)
; ili ako je serija neuspješna (3 neispravna proizvoda).
...
to go
  ...
  [
    ...
    set history sort history ;; sortira se povijest opazanja kako bi 0 (neispravni proizvodi) dosli na pocetak
    check-history            ;; funkcija provjere povijesti opazanja

    if (br-ispravno = 30) ; serija proizvoda sadrži 30 proizvoda
    [
      show (word "Uspješna proizdvodnja!")       ; ukoliko je manje od 10% neispravnih serija je uspjela
      stop
    ]
    tick
  ]
...
to walk ;; funkcija pomicanja proizvoda po traci
  ...
  [
    ...
    [
      ifelse color = red
      [
        set br-neispravno (br-neispravno + 1)
        show (word "Neispravan proizvod: " br-neispravno)
        set history lput 0 history
        set-xy-bad
      ]
      [
        set br-ispravno (br-ispravno + 1)
        set history lput 1 history
        show (word "Ispravan proizdvod: " br-ispravno)
        die   ; ako je ispravan proizvod - ukloni sa ekrana
      ]
    ]
      fd 1
  ]
...
to new-product
  ...
  [
    ...
    set shape "box"    ;; daje mu se oblik kutije
...
to check-history
  ...
  [
    if (item 2 history = 0)      ;; ako je trece (indeks 2) opazanje u sortiranoj listi 0, tj. neispravan proizvod
    [
      ask patch 6 6
      [
        set pcolor red    ;; signalno svjetlo se postavlja na crvenu boju
      ]
      show "PREKID! 10% proizvodnje je neispravno."         ;; ukoliko je 10% i vise neispravnih serija nije uspjela
...
Nakon modifikacije, automatski se stvaraju novi proizvodi u trenutku kada je svaki TICK djeljiv sa brojem 21 .
Pojavljivanje novih proizvoda se zaustavlja ako je proizvodnja uspješna (30 ispravnih proizvoda) ili ako su u procesu proizvodnje nastala 3 neispravna proizvoda.   
